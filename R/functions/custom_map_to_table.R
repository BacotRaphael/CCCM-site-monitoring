custom_map_to_table <- function (result) 
{
  if (is.null(result)) {
    return(result)
  }
  ...
  nums_formated <- round(result$summary.statistic$numbers, 
                         2)
  if (is_percent) {
  ...
  if (has_dependent_var_values) {
    cols_to_keep <- c(cols_to_keep, "dependent.var.value")
  }
  if (has_independent_var_values) {
    cols_to_keep <- c(cols_to_keep, "independent.var.value")
  }
  table_to_show <- result$summary.statistic[, cols_to_keep, 
                                            drop = FALSE]
  if (has_dependent_var_values) {
    table_to_show <- tidyr::spread(table_to_show, key = "dependent.var.value", 
                                   value = numbers)
  }
  else {
    colnames(table_to_show)[colnames(table_to_show) == "numbers"] <- ifelse(is_percent, 
                                                                            "%", "Average")
  }
  if (has_independent_var_values) {
    colnames(table_to_show)[colnames(table_to_show) == "independent.var.value"] <- result$summary.statistic$independent.var
  }
